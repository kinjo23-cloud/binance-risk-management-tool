...
def executebuy_trade():
    ...
    logger.info("Starting buy order script...")
    ...
    sl_distance = 6.5  # in price units (e.g., $6.5 stop loss)
    tp_distance = sl_distance * 2
    sl_price = round(price - sl_distance, 1)
    tp_price = round(price + tp_distance, 1)

    # ✅ New correct logic:
    # Risk = sl_distance * quantity => quantity = sl_usdt / sl_distance
    timestamp = get_server_time()
    ...
    # Calculate required margin with 1% buffer
    required_margin = position_value/leverage  # Apply buffer after applying leverage
